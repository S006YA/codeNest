...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Additional origins can be configured here
      return false;
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="developer" -c user.email="dev@aeterna.local" add "${targetFile}"`);
            execSync(`git -c user.name="developer" -c user.email="dev@aeterna.local" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
